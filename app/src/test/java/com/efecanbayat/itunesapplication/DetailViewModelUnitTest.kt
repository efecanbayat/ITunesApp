...
import com.efecanbayat.itunesapplication.ui.detailscreen.DetailViewModel
...
class DetailViewModelUnitTest {
    ...
    private val data = parseFileAsBaseResponse("fakeDetailResponse.json")
    ...
    private var detailStateObserver = mock<Observer<Resource<Any>>>()
    ...
    fun setUp() {
        ...
        detailViewModel = DetailViewModel(apiRepository)
    }

    @Test
    fun `get live data from viewModel and check is successful`() = coroutineTestRule.runBlockingTest {

        whenever(apiService.getDetailById(anyString())).thenReturn(
            mockSuccessResponse
        )

        detailViewModel.getDetailById("499334149").observeForever(detailStateObserver)
        detailViewModel.getDetailById("499334149").getOrAwaitValue()

        verify(detailStateObserver).onChanged(Resource.loading())
        verify(detailStateObserver).onChanged(Resource.success(data))
    }

    @Test
    fun `get live data from viewModel and check is unsuccessful`() = coroutineTestRule.runBlockingTest {
        ...
        verify(detailStateObserver).onChanged(Resource.loading())
        verify(detailStateObserver).onChanged(Resource.error("Error: ERROR", null))
    }
}